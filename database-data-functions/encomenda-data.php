...
function createOrder($data, $local, $desconto, $status, $cliente, $total, $totalDesconto) {
    ...
    if($desconto == null) {
        ...
        $query = pg_query(getDBConnection(), "INSERT INTO encomenda(data_encomenda, local_entrega, desconto_id, status_id, cliente_utilizador_username, preco_total, total_desconto)
                                                    VALUES ('" . $data . "', '" . $local . "', '" . $desconto . "', '" . $status . "', '" . $cliente . "', '" . $total . "', '" . $totalDesconto . "')");
    }
...
function getOrderFoodByOrderId($id) {
    return pg_fetch_all(pg_query(getDBConnection(),
        "SELECT * 
              FROM encomenda, encomenda_comida 
